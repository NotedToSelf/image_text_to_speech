...
for tensorInput, tensorTarget in objectDataset.dataset:
	#Wrap tensor in variable
	Input = torch.autograd.Variable(data=tensorInput)
	Input = Input.unsqueeze(1)
	realnum = tensorTarget 

	estimate = moduleNetwork(Input)
	num, ind = torch.max(estimate, 1)
	Input = Input.squeeze(1)
	if (int(ind) != int(realnum)):
		print(str(int(ind)) + " " + str(int(realnum)))
		write_image(Input.data, int(realnum), int(ind))
